...
import hashlib      # SHA-256 для хешування user_id перед записом у CSV (урок 5)
...
# ── Сіль для хешування user_id (урок 5: crypto_basics.py) ──
# Замість того щоб писати в bot_log.csv plain user_id (персональні дані),
# зберігаємо sha256(salt + user_id). Сіль — у .env, у git не потрапляє.
# Якщо USER_ID_SALT не задано → лог пише plain user_id з попередженням.
# Згенерувати сіль можна так: python -c "import secrets; print(secrets.token_hex(32))"
USER_ID_SALT = os.getenv("USER_ID_SALT", "").strip()


def hash_user_id(uid: int) -> str:
    """Повертає sha256(salt + user_id) у hex — детально див. урок 5 (crypto_basics.py)."""
...
def log_request(user_id: int, username: str, city: str, temp: float, feels_like: float, description: str):
    ...
    user_id хешуємо через sha256(salt + user_id) — це урок 5 (crypto_basics.py).
    Захешований ID:
      • стабільний (однакові id → однакові хеші → /stats nunique() працює коректно),
      • необоротний (з логу неможливо відновити плейн user_id).
    """
    with open(LOG_FILE, "a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            datetime.now().isoformat(),  # поточний час у форматі ISO 8601
            hash_user_id(user_id),       # SHA-256(salt + user_id) — приватність (урок 5)
            username,
            city,
            temp,
            feels_like,
...
def get_user_summary_sql(user_id: int):
    """Повертає особисту статистику юзера через JOIN трьох таблиць.

    Демонстрація уроку №4 (sql_join.py): JOIN weather_log + users + cities.
    Запит з'єднує всі три таблиці й повертає для конкретного user_id:
        username, language, home_city (з users),
        кількість запитів (з weather_log),
        улюблений регіон (через підзапит з cities).

    Повертає dict або None у двох випадках:
        • БД недоступна / помилка запиту,
        • юзера немає в таблиці users (її наповнює sql_join.py).
    """
    try:
        conn = get_db_conn()
        with conn.cursor() as cur:
            # LEFT JOIN, щоб юзер без запитів усе одно з'явився з COUNT = 0.
            # Підзапит у SELECT — окрема міні-агрегація: знайти топ-регіон саме
            # цього юзера. Для цього потрібен JOIN з cities — те, що ми
            # вчили на КРОЦІ 4 уроку sql_join.py.
            cur.execute(
                """
                SELECT
                    u.username,
        ...
        logger.warning(f"⚠️  PostgreSQL запит /me впав: {e}")
        return None
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/me — особиста статистика через SQL JOIN\n"
        "/help — допомога"
    )
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/me — особиста статистика через JOIN трьох таблиць"
    )
...
async def me(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Команда /me — особиста статистика юзера через JOIN трьох таблиць.

    ╔══════════════════════════════════════════════════════════════╗
    ║  Це урок №4 (sql_join.py) у дії!                             ║
    ║  Запит з'єднує weather_log + users + cities, щоб для         ║
    ║  поточного юзера показати:                                   ║
    ║     • username (з users)                                     ║
    ║     • language та home_city (з users)                        ║
    ║     • кількість запитів (COUNT з weather_log)                ║
    ║     • улюблений регіон (підзапит з JOIN cities)              ║
    ║                                                              ║
    ║  Стрижень запиту:                                            ║
    ║      FROM users u                                            ║
    ║      LEFT JOIN weather_log w ON u.user_id = w.user_id        ║
    ║                                                              ║
    ║  Юзера може не бути в users — у такому випадку повертаємо    ║
    ║  підказку запустити sql_join.py (там тестовий набір з 6).    ║
    ╚══════════════════════════════════════════════════════════════╝
    """
    user = update.effective_user
    info = get_user_summary_sql(user.id)

    if info is None:
        await update.message.reply_text(
            "ℹ️  Тебе ще немає в таблиці `users` (PostgreSQL) — "
            "або БД недоступна.\n\n"
            "Запусти `python sql_join.py` — він створить демо-таблиці "
            "`users` та `cities` з 6-ма юзерами, і ти зможеш побачити "
            "як працює JOIN на тестових даних."
        )
        return

    home = info["home_city"] or "—"
    top_region = info["top_region"] or "—"
    text = (
        f"👤 Профіль (SQL JOIN трьох таблиць)\n"
        f"{'─' * 30}\n"
        f"Юзер: {info['username']}\n"
        f"Мова: {info['language']}\n"
        f"Home city: {home}\n"
        f"Запитів: {info['requests']}\n"
        f"Улюблений регіон: {top_region}\n\n"
        f"💡 Цей результат — це JOIN таблиць users + weather_log + cities."
    )
    await update.message.reply_text(text)
...
def main():
    ...
    # ── Урок 5: попередження, якщо USER_ID_SALT порожній ──
    # Бот працює і без солі, але тоді хеш є тривіально-передбачуваним
    # (sha256(str(user_id))). Це краще, ніж plain user_id, але набагато слабше
    # за хеш з солі. Просимо додати USER_ID_SALT у .env.
    if not USER_ID_SALT:
        logger.warning(
            "⚠️  USER_ID_SALT не задано в .env — у логи піде sha256 без солі. "
            "Згенеруй сіль: python -c \"import secrets; print(secrets.token_hex(32))\""
        )
